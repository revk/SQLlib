...
SQL *sql_real_connect(MYSQL * sql, const char *host, const char *user, const char *passwd, const char *db, unsigned int port, const char *unix_socket, unsigned long client_flag, char safe, const char *mycnf)
{                               // Connect but check config file
   ...
   } else
   {
      ...
      sql_set_character_set(s, "utf8"); // Seems needed fro mariadb
   }
   return s;
}

int sql_safe_select_db(SQL * sql, const char *db)
{
   if (sqlsyslogquery >= 0)
...
void sql_safe_query_free(SQL * sql, char *q)
{
   if (!q)
      return;
   sql_safe_query(sql, q);
...
SQL_RES *sql_safe_query_use_free(SQL * sql, char *q)
{
   if (!q)
      return 0;
...
SQL_RES *sql_safe_query_store_free(SQL * sql, char *q)
{
   if (!q)
      return 0;
...
SQL_RES *sql_query_use_free(SQL * sql, char *q)
{
   if (!q)
      return 0;
...
SQL_RES *sql_query_store_free(SQL * sql, char *q)
{
   if (!q)
      return 0;
...
int sql_query_free(SQL * sql, char *q)
{
   if (!q)
      return 0;
...
char *sql_safe_query_value(SQL * sql, char *q)
...
char *sql_safe_query_value_free(SQL * sql, char *q)
{                               // does query, returns strdup of first column of first row of result, or NULL
   if (!q)
      return NULL;
...
const char *sql_colname(SQL_RES * res, int c)
{
   return res->fields[c].name;
}

char *sql_col(SQL_RES * res, const char *fieldname)
...
SQL_FIELD *sql_col_format(SQL_RES * res, const char *fieldname)
...
time_t sql_time_z(const char *t, int utc)
