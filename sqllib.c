...
SQL *sql_real_connect(MYSQL * sql, const char *host, const char *user, const char *passwd, const char *db,
                      unsigned int port, const char *unix_socket, unsigned long client_flag, char safe,
                      const char *mycnf)
      ...
      switch (*f)
      {
        ...
        case 'Z':              // time (utc)
        {
          ...
          else
          {
            ...
            int l =
              sprintf(s->query + s->ptr, "%04u-%02u-%02u %02u:%02u:%02u", t->tm_year + 1900, t->tm_mon + 1, t->tm_mday,
...
{                               // time_t for sql date[time], -1 for error, 0 for 0000-00-00 00:00:00
  if (!t)
    return -1;
  unsigned int Y = 0, M = 0, D = 0, h = 0, m = 0, s = 0, c;
  c = 4;
  while (isdigit(*t) && c--)
    Y = Y * 10 + *t++ - '0';
  if (*t == '-')
