...
{                               // Connect but check config file
   ...
   if (sqlcnf && *sqlcnf)
   {
      char *fn = (char *) sqlcnf;
      if (*fn == '~')
         if (asprintf (&fn, "%s%s", getenv ("HOME") ? : "/etc", fn + 1) < 0)
            errx (1, "malloc");
      struct stat s;
   ...
   if(host&&!*host)host=NULL;	// A blank host as local connection
...
{                               // Formatted print, append to query string
   while (*f)
   {
      ...
      if (!strchr ("diouxXeEfFgGaAcsCSpnmTBZ", *f) || f - base > 20)
